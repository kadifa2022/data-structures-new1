...
@Override
    public String toString() {//runtime polymorphism// on ly one implementation get executed
        return "Pizza{" +
                "size=" + size +
    ...
    @Override
    public boolean equals(Object obj) {//Object is parent of pizza class // equals() is from object class
         //instanceOf() is to identify object type
        if( !(obj instanceof Pizza )){ // if the specified object is not pizza
            System.err.println("Invalid object: "+obj);
            System.exit(1);
        }
        //casted
        if(size ==  ( (Pizza)obj ).size  ){//pizza.getSize()
            if( numberOfPepperoniTopping == ( (Pizza)obj ).numberOfPepperoniTopping){//pizza.getNumberOfPeperoniTopping()
                return true;
            }
